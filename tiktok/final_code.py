    ...
    for profile_url in initial_profiles:
        ...
        if "videos" in videos_data:
            for video in videos_data["videos"]:
                ...
                if video_id:
                    # Create a dedicated folder for each video
                    video_dir = os.path.join(user_dir, f"video_{video_id}")
                    os.makedirs(video_dir, exist_ok=True)
                    
                    # Save the video details in this folder
                    video_details_file = os.path.join(video_dir, "video_details.json")
                    await save_json(video_details_file, video)
                    
                    # Create a subfolder for comments within the video folder
                    video_comments_dir = os.path.join(video_dir, "comments")
                    os.makedirs(video_comments_dir, exist_ok=True)
                    
                    # Query comments for the video
                    ...
                    # Save the comments in the dedicated subfolder
                    comments_file = os.path.join(video_comments_dir, "comments.json")
